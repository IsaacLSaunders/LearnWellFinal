<template>
  <nav class="breadcrumb is-medium is-centered" aria-label="breadcrumbs">
    <ul>
      ...
      <li>
        <a v-on:click="backToCourse(courseData.courseId)">{{
          this.courseData.courseName
        }}</a>
      </li>
      <li class="is-active">
        <a href="#" aria-current="page">{{ curriculum.description }}</a>
      </li>
    </ul>
  </nav>
  ...
  <div class="box">
    <h1 class="is-size-1">
      <strong>{{ this.curriculum.description }} </strong>
    </h1>
    ...
    <div class="column is-one-quarter">
      <button
        ...
        v-on:click="
          goToEditCurriculum(
            courseData.courseId,
            this.curriculum.curriculumElementId
          )
        "
      >
        Edit curriculum
    ...
    <div class="card-content has-background-link-light">
      <div class="content">
        <table
          class="table is-hoverable is-fullwidth has-background-link-light"
        >
          <h2 class="is-size-4">Assignments</h2>
          <tbody>
            <tr
              v-for="assignment in this.curriculum.assignments"
  ...
  methods: {
    ...
    goToEditCurriculum(courseId, curriculumElementId) {
      this.$router.push({
        ...
        params: { courseId: courseId, elementId: curriculumElementId },
      });
    },
  },
